...
<!-- BEGIN: Page JS auto complete-->
<script src="<?= base_url() ?>/assets/js/easy-autocomplete/jquery.easy-autocomplete.js" type="text/javascript"></script>
<!-- END: Page JS-->

<script src="<?= base_url() ?>/assets/vendors/js/tables/datatable/datatables.min.js" type="text/javascript"></script>
<!-- END: Page Vendor JS-->

<script src="<?= base_url() ?>/assets/js/scripts/tables/datatables/datatable-basic.min.js"
		type="text/javascript"></script>
<script src="<?= base_url() ?>/assets/js/main/cariDosen.js" type="text/javascript"></script>
<script src="<?= base_url() ?>/assets/js/apps.js" type="text/javascript"></script>

<script src="<?= base_url() ?>/assets/js/core/sweetalert2.all.js" type="text/javascript"></script>
<script src="<?= base_url() ?>/assets/js/core/sweetalert2.min.js" type="text/javascript"></script>

<script src="<?= base_url() ?>assets/js/core/jQuery.countdownTimer.js"></script>

<script src="<?= base_url() ?>assets/css/plugins/summernote/dist/summernote-bs4.js" type="text/javascript"></script>

<script type="text/javascript">
	$(document).ready(function () {
		$('#btn-submit-soal').click(function () {
			$('#formSoal').submit();
		});
	});
</script>
<?php if ($this->session->flashdata('alert') == 'sudah_masuk_kelas') { ?>
	<script>
		Swal.fire(
			'Sudah Masuk kelas',
			'',
...
<?php } ?>
<?php if ($this->session->flashdata('alert') == 'kelas_tidak_ada') { ?>
	<script>
		Swal.fire(
			'kelasnya nggak ada sis',
			'',
			'error'
		)
	</script>
<?php } ?>
<?php if ($this->session->flashdata('alert') == 'simpan_data') { ?>
	<script>
		Swal.fire(
...
<?php } ?>
<?php if ($this->session->flashdata('alert') == 'data_tidak_tersimpan') { ?>
	<script>
		Swal.fire(
			'Data tidak disimpan',
			'',
...
<?php } ?>
<?php if ($this->session->flashdata('alert') == 'ubah_data') { ?>
	<script>
		Swal.fire(
...
<?php } ?>
<?php if ($this->session->flashdata('alert') == 'tidak_terubah') { ?>
	<script>
		Swal.fire(
			'berhasil mengubah data',
			'',
...
<?php } ?>
<?php if ($this->session->flashdata('alert') == 'hapus_data') { ?>
	<script>
		Swal.fire(
...
<?php } ?>
<?php if ($this->session->flashdata('alert') == 'ubahsub_data') { ?>
	<script>
		Swal.fire(
			'berhasil mengubah data',
			'',
...
<?php } ?>
<?php if ($this->session->flashdata('alert') == 'hapussub_data') { ?>
	<script>
		Swal.fire(
			'berhasil menghapus data',
			'',
...
<!-- Mirrored from themeselection.com/demo/chameleon-admin-template/html/ltr/vertical-menu-template/ by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 30 May 2019 10:14:21 GMT -->
</html>
